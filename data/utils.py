...
class BdentalConstants():
    """Bdental constants"""

    BDENTAL_LIB_NAME='Bdental_Library'
    BDENTAL_LIB_ARCHIVE_NAME='Bdental_Library_Archive'
    ADDON_VER_NAME = "BDENTAL_Version.txt"
    BDENTAL_MODULES_NAME = "bdental_modules"
    ...
    ADDON_VER_DATE = get_bdental_version(filepath=ADDON_VER_PATH)
    ...
    BDENTAL_MODULES_PATH = join(ADDON_DIR, BDENTAL_MODULES_NAME)
    MESH_REG_AUTO_PATH = join(ADDON_DIR, "operators","MeshRegAuto.exe")
    DATA_BLEND_FILE = join(RESOURCES, "BlendData","BDENTAL_BlendData.blend")
    BDENTAL_APP_TEMPLATE_PATH = join(RESOURCES, "bdental_app_template.zip")
    ...
    MAIN_WORKSPACE_NAME = "Bdental Main"
    SLICER_WORKSPACE_NAME = "Bdental Slicer"
    ...
    BDENTAL_VOXEL_SHADER = "VGS_Dakir_(-400-3000)"#"VGS_Dakir_01"#"VGS_Dakir_MinMax"
    GUIDE_COMPONENTS_COLLECTION_NAME = "Surgical_Guide_Components_Collection"
    BOOL_NODE ="boolean_geonode"
    GUIDE_NAME = "Bdental_guide"
    BDENTAL_IMAGE_METADATA_TAG = "bdental_image"
    BDENTAL_IMAGE_METADATA_KEY = "image_type"
    BDENTAL_IMAGE_METADATA_UID_KEY = "uid"
    WMIN = -400
    WMAX = 3000
    BONE_UINT_THRESHOLD = 5
    ...
    IMPLANT_SAFE_ZONE_TYPE = "bdental_implant_safe_zone"
    IMPLANT_SLEEVE_TYPE = "bdental_implant_sleeve"
    IMPLANT_PIN_TYPE = "bdental_implant_pin"
    FIXING_PIN_TYPE = "bdental_fixing_pin"
    FIXING_SLEEVE_TYPE = "bdental_fixing_sleeve"

    MHA_PATH_TAG = "mha_path"
    SOURCE_MHA_TAG = "source_mha"
    BDENTAL_TYPE_TAG = "bdental_type"
    IMPLANT_LOCKED_MAT_NAME = "implant_locked_mat"
    IMPLANT_MAT_TAG = "implant_mat"
    BDENTAL_IMPLANT_REMOVE_CODE_TAG = "bdental_remove_code"
...
class BDENTAL_OT_BdentalModulesPipInstaller(bpy.types.Operator):
    """pip install bdental modules"""

    bl_idname = "wm.bdental_modules_pip_installer"
    bl_label = "Install Bdental Modules Online"
    bl_options = {"REGISTER"}
    ...
    def execute(self, context):
        if not isConnected(debug=True):
            ...
            return {"CANCELLED"}
        target_dir_path = BdentalConstants.BDENTAL_MODULES_PATH
    ...
    def invoke(self, context, event):
        ...
        if not self.modules_list:
            message = ["Bdental modules already installed!"]
            bdental_log(message)
...
def pip_install_modules(modules_list, target_dir_path=None):
    ...
    BDENTAL_GpuDrawText(message_list=[info_text])
    try :
        subprocess.check_call([sys.executable, "-m", "pip", "install", "-U", "pip"])
    except subprocess.CalledProcessError as e:
        info_text = f"Updating pip failed!"
        bdental_log([f"Updating pip failed! : {e}"])
    ...
    for module in modules_list:
        ...
        try:
            ...
            BDENTAL_GpuDrawText(message_list=[info_text],rect_color=BdentalColors.red)
            error_message.append(module)
    return error_message

def get_incremental_idx(data =None,bdental_type=None):
    
    idx = 0
    if data is not None :
        objects = data
        if bdental_type :
            objects = [obj for obj in data if obj.get("bdental_type") == bdental_type]
            if objects :
                idx = len(objects) + 1
    return idx
...
def get_update_version(filepath=BdentalConstants.UPDATE_VERSION_URL)  :
    success = False
    txt_list= []
    update_version = None
    try :
        ...
        success = r.ok
        if not success :
            bdental_log([f"connection to github update server failed! :\n{r.text}"])
...
def add_bdental_libray() :
    message = []
    success = 0
    if not exists(BdentalConstants.BDENTAL_LIBRARY_ARCHIVE_PATH):
        message = ["Bdental Library archive not found"]
        return message,success
    _libs_collection_group = bpy.context.preferences.filepaths.asset_libraries
    bdental_lib = _libs_collection_group.get(BdentalConstants.BDENTAL_LIB_NAME)
    if bdental_lib :
        idx = [i for i,l in enumerate(_libs_collection_group) if l.name==BdentalConstants.BDENTAL_LIB_NAME][0]
        bpy.ops.preferences.asset_library_remove(index=idx)
    
    if exists(BdentalConstants.BDENTAL_LIBRARY_PATH):
        shutil.rmtree(BdentalConstants.BDENTAL_LIBRARY_PATH)
    os.mkdir(BdentalConstants.BDENTAL_LIBRARY_PATH)

    archive_list = os.listdir(BdentalConstants.BDENTAL_LIBRARY_ARCHIVE_PATH)
    for _item in archive_list :
        _item_full_path = join(BdentalConstants.BDENTAL_LIBRARY_ARCHIVE_PATH,_item)
        if _item.endswith('.zip') :
            with zipfile.ZipFile(_item_full_path, 'r') as zip_ref:
                zip_ref.extractall(BdentalConstants.BDENTAL_LIBRARY_PATH)
        else :
            shutil.copy2(_item_full_path, BdentalConstants.BDENTAL_LIBRARY_PATH)

    bpy.ops.preferences.asset_library_add(directory=BdentalConstants.BDENTAL_LIBRARY_PATH)
    success = 1
    return message, success

def import_required_modules(required_modules_dict=BdentalConstants.REQ_DICT):
    missing_modules = []
...
def set_modules_path(modules_path=BdentalConstants.BDENTAL_MODULES_PATH):
    if not modules_path in sys.path :
        sys.path.insert(0,BdentalConstants.BDENTAL_MODULES_PATH)

    
def addon_update_preinstall(update_root):
    
    update_data_map_json = join(update_root, BdentalConstants.UPDATE_MAP_JSON)
    ...
    for src,dst in update_data_dict.items():
        ...
        if BdentalConstants.BDENTAL_MODULES_NAME in src.lower() :
            shutil.move(src, BdentalConstants.RESOURCES)
        else :
...
def addon_update_download():
    ...
    try :
        ...
        os.chdir(temp_dir)
        _update_zip_local = join(temp_dir,f'{BdentalConstants.ADDON_UPDATE_NAME}.zip')

        # Download the file
        with requests.get(BdentalConstants.ADDON_UPDATE_URL, stream=True, timeout=10) as r:
            try:
                r.raise_for_status()
                ...
                txt = f"Error during requests to {BdentalConstants.ADDON_UPDATE_URL}"
    ...
    try :
        ...
        update_root = join(temp_dir,BdentalConstants.ADDON_UPDATE_NAME)
        ...
        bdental_log([f"Error occurred while extracting the downloaded addon ZIP file: {err}"]) 
...
def HuTo255(Hu, Wmin=BdentalConstants.WMIN, Wmax= BdentalConstants.WMAX):
...
class BDENTAL_GpuDrawText() :
    ...
    def __init__(self,
                ...
                txt_size = BdentalConstants.BLF_INFO.get("size"),
                btn_txt = "OK",
                info_handler = None,
                sleep_time=0
                ):
    ...
    def _cancell_previous(self):
        for _h in DRAW_HANDLERS:
            try :
                ...
                bdental_log([f"remove_handler error : {er}"])
    ...
    def gpu_info_footer(self):
        ...
        def draw_callback_function():
            ...
            for i, txt in enumerate((reversed(self.message_list))):
                self.draw_gpu_rect(self.offset_horizontal, self.offset_vertical + self.line_height+2+(self.rect_height*i), w_full-self.offset_horizontal, self.rect_height, BdentalColors.base)
                blf.position(BdentalConstants.BLF_INFO.get('fontid'), self.offset_horizontal+2, self.offset_vertical + self.line_height+2+2+(self.rect_height*i), 0)
                blf.size(BdentalConstants.BLF_INFO.get("fontid"), self.txt_size) # 3.6 api blf.size(0, 40, 30) -> blf.size(fontid, size)
