...
class BDENTAL_OT_AssetBrowserToggle(bpy.types.Operator):
    ...
    def defer(self):
        ...
        if not params:
            return 0
        try:
            params.asset_library_reference = BdentalConstants.BDENTAL_LIB_NAME 
    ...
    def execute(self, context):
        ...
        if success :
            ...
            with context.temp_override(window=context.window_manager.windows[0]):
                BDENTAL_GpuDrawText(message_list=["Please restart to apply Bdental Configuration."],
                                     rect_color=BdentalColors.green, sleep_time=3)
                
        else :
            with context.temp_override(window=context.window_manager.windows[0]):

                BDENTAL_GpuDrawText(message_list=["Bdental Configuration failed."],
                                     rect_color=BdentalColors.red, sleep_time=2)
        return{"FINISHED"}
...
class BDENTAL_OT_AddAppTemplate(bpy.types.Operator):
    ...
    def execute(self, context):
        try :
            bpy.ops.preferences.app_template_install( filepath=BdentalConstants.BDENTAL_APP_TEMPLATE_PATH)
...
class BDENTAL_OT_add_3d_text(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        self.text_ob[BdentalConstants.BDENTAL_TYPE_TAG] = "bdental_text"
...
class BDENTAL_OT_OpenManual(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        Manual_Path = join(BdentalConstants.ADDON_DIR, "Resources", "BDENTAL User Manual.pdf")
...
class BDENTAL_OT_NewProject(bpy.types.Operator):
    ...
    bl_idname = "wm.bdental_new_project"
...
class BDENTAL_OT_DrawTester(bpy.types.Operator):
    """test for gpu info footer draw"""

    bl_idname = "wm.bdental_draw_tester"
    bl_label = "Draw Test"
    ...
    def execute(self, context):
        ...
        for i, message in enumerate([message1, message2, message3]):
            BDENTAL_GpuDrawText(message_list=message, rect_color=BdentalColors.default, sleep_time=3, percentage=20+i*40)
        return {"FINISHED"}
...
class BDENTAL_OT_Dicom_Reader(bpy.types.Operator):
    """Dicom Reader"""

    bl_idname = "wm.bdental_dicom_reader"
    bl_label = "Dicom Read"
    ...
    def draw(self, context):
        ...
        if self._props.visualisation_mode == BdentalConstants.VISUALISATION_MODE_PCD:
    ...
    def get_sitk_image_from_singlFile(self,context):
        ...
        try:
            ...
            is_bdental_image = False
            is_other_valid_image = False

            
            
            if self.sitk_image.HasMetaDataKey(BdentalConstants.BDENTAL_IMAGE_METADATA_KEY):
                print(f"bdental image pixel id type : {self.sitk_image.GetPixelIDTypeAsString()}")
                image_type = self.sitk_image.GetMetaData(BdentalConstants.BDENTAL_IMAGE_METADATA_KEY)  # Image Type
                is_bdental_image = (image_type == BdentalConstants.BDENTAL_IMAGE_METADATA_TAG)
                image_uid = self.sitk_image.GetMetaData(BdentalConstants.BDENTAL_IMAGE_METADATA_UID_KEY)
            
            elif self.dicom_data_path.endswith("_Image3D255.nrrd") and \
                self.sitk_image.GetPixelIDTypeAsString() == "8-bit unsigned integer":
                is_bdental_image = True
            ...
            if not (is_bdental_image or is_other_valid_image):
            ...
            dicom_dictionary.update({
                ...
                "uid": image_uid,
                "is_bdental_image": is_bdental_image,
            })
    ...
    def invoke(self, context, event):
        ...
        self._props = context.scene.BDENTAL_Props
        ...
        BDENTAL_GpuDrawText(message_list=message, percentage=pourcentage)
        #################################################################
    ...
    def fix_sitk_image(self):
        self.sitk_image = sitk.Cast(
                    sitk.IntensityWindowing(
                        self.sitk_image,
                        windowMinimum=BdentalConstants.WMIN,
                        windowMaximum=BdentalConstants.WMAX,
                        outputMinimum=0,
                        outputMaximum=255,
                    ),
                    sitk.sitkUInt8)
    ...
    def get_main_image(self, context):
        ...
        if self.dicom_data_type == "single_file" :
            ...
            is_bdental_image = self.current_dicom_cach_dictionary.get("is_bdental_image")
            if not is_bdental_image :
                self.fix_sitk_image()
        ...
        bdental_metadata_dict = {BdentalConstants.BDENTAL_IMAGE_METADATA_KEY: BdentalConstants.BDENTAL_IMAGE_METADATA_TAG, BdentalConstants.BDENTAL_IMAGE_METADATA_UID_KEY: self.uid}
        for key, value in bdental_metadata_dict.items():
        ...
        context.scene[BdentalConstants.MHA_PATH_TAG] = self.mha_path = RelPath(mha_file_path)
        ...
        self.current_dicom_cach_dictionary[BdentalConstants.MHA_PATH_TAG] = RelPath(mha_file_path)
        self._props.current_dicom_dictionary = str(self.current_dicom_cach_dictionary)

        
        # image = sitk.ReadImage(mha_file_path)
        # keys_to_check = [BdentalConstants.BDENTAL_IMAGE_METADATA_KEY, BdentalConstants.BDENTAL_IMAGE_METADATA_UID_KEY]
        # found = {}
    ...
    def point_cloud_visualization(self, max_points):
        ...
        """
        bdental_log(["BDENTAL_OT_Dicom_Reader.point_cloud_visualization()"])
        start = tpc()
        error_message = []
        pcd_idx = get_incremental_idx(data=bpy.data.objects,bdental_type=BdentalConstants.PCD_OBJECT_TYPE)
        pcd_object_name = get_incremental_name(BdentalConstants.PCD_OBJECT_NAME,pcd_idx)
        pcd_geonode_name = get_incremental_name(BdentalConstants.PCD_GEONODE_NAME,pcd_idx)
        pcd_geonode = bpy.data.node_groups.get(pcd_geonode_name)
        if not pcd_geonode :
            filepath = join(BdentalConstants.DATA_BLEND_FILE, "NodeTree", BdentalConstants.PCD_GEONODE_NAME)
            directory = join(BdentalConstants.DATA_BLEND_FILE, "NodeTree")
            filename = BdentalConstants.PCD_GEONODE_NAME
            bpy.ops.wm.append(filepath=filepath,
                            filename=filename, directory=directory)
            pcd_geonode = bpy.data.node_groups.get(BdentalConstants.PCD_GEONODE_NAME)
            pcd_geonode.name = pcd_geonode_name
            
        pcd_geonode["uid"] = self.uid

        viz_collection = bpy.data.collections.get(BdentalConstants.DICOM_VIZ_COLLECTION_NAME)
        if viz_collection:
            hide_collection(_hide=False, colname=BdentalConstants.DICOM_VIZ_COLLECTION_NAME)
        ...
        for scr in bpy.data.screens:
            ...
            for area in areas:
                ...
                for space in spaces:
                    ...
                    r3d.view_matrix = Matrix(BdentalConstants.VIEW_MATRIX)
                    # space.shading.color_type = "TEXTURE"
                    r3d.update()
        """
        import bpy

        #scr = bpy.data.screens["Bdental Main"]
        ...
        bdental_log([f" scene preparation : {tpc() - start} seconds"])
        step = tpc()
        ...
        MoveToCollection(pcd_obj, BdentalConstants.DICOM_VIZ_COLLECTION_NAME)

        bdental_log([f" write pcd_mesh from numpy : {tpc() - step} seconds"])
        step = tpc()

        # Create integer threshold attribute
        attr = pcd_obj.data.attributes.new(name=BdentalConstants.PCD_INTENSITY_ATTRIBUTE_NAME, type='INT', domain='POINT')
        attr.data.foreach_set("value", intensities)
        # for i, value in enumerate(intensities):
        #     attr.data[i].value = value 

        bdental_log([f" set pcd_mesh intensity attribute : {tpc() - step} seconds"])
        step = tpc()
        # Copy Geometry Nodes modifier
        pcd_modifier = pcd_obj.modifiers.get(BdentalConstants.PCD_GEONODE_MODIFIER_NAME) or \
        pcd_obj.modifiers.new(name=BdentalConstants.PCD_GEONODE_MODIFIER_NAME, type='NODES')
        pcd_modifier.node_group = pcd_geonode 

        # Update display
        
        pcd_obj[BdentalConstants.BDENTAL_TYPE_TAG] = BdentalConstants.PCD_OBJECT_TYPE
        pcd_obj[BdentalConstants.MHA_PATH_TAG] = self.mha_path
        pcd_obj["idx"] = pcd_idx
        pcd_obj["uid"] = self.uid
        pcd_obj[BdentalConstants.PCD_GEONODE_NAME_TAG] = pcd_geonode_name
        ...
        bdental_log([f" total time : {tpc() - start} seconds"])
    ...
    def voxel_visualization (self,context):
        ...
        voxel_node_name = get_incremental_name(BdentalConstants.VOXEL_GROUPNODE_NAME,voxel_idx)
        voxel_node = bpy.data.node_groups.get(voxel_node_name)
        if not voxel_node :
            filepath = join(BdentalConstants.DATA_BLEND_FILE, "NodeTree", BdentalConstants.BDENTAL_VOXEL_SHADER)
            directory = join(BdentalConstants.DATA_BLEND_FILE, "NodeTree")
            filename = BdentalConstants.BDENTAL_VOXEL_SHADER
            bpy.ops.wm.append(filepath=filepath,
                            filename=filename, directory=directory)
            voxel_node = bpy.data.node_groups.get(BdentalConstants.BDENTAL_VOXEL_SHADER)
        ...
        voxel_plane_mesh_name = get_incremental_name(f"{BdentalConstants.VOXEL_PLANE_NAME}_mesh",voxel_idx)
        voxel_plane_mesh = AddPlaneMesh(dim_x, dim_y, voxel_plane_mesh_name)
        
        voxel_collection = bpy.data.collections.get(BdentalConstants.DICOM_VIZ_COLLECTION_NAME)
        if voxel_collection:
            hide_collection(_hide=False, colname=BdentalConstants.DICOM_VIZ_COLLECTION_NAME)
        ...
        for i in range(num_slices):
            ...
            # alpha_channel = np.ones_like(voxel_array)*(voxel_array>=BdentalConstants.BONE_UINT_THRESHOLD)*255
            alpha_channel = np.ones_like(voxel_array).astype(np.float32)*(voxel_array>=BdentalConstants.BONE_FLOAT_THRESHOLD)
            voxel_array_with_alpha = np.stack([voxel_array, voxel_array, voxel_array, alpha_channel], axis=-1)
            voxel_image_name = get_incremental_name(BdentalConstants.VOXEL_IMAGE_NAME,voxel_idx,i)
            voxel_image_data = create_blender_image_from_np(voxel_image_name, voxel_array_with_alpha)
            voxel_image_data[BdentalConstants.BDENTAL_TYPE_TAG] = BdentalConstants.VOXEL_IMAGE_TYPE

            voxel_plane_name = get_incremental_name(BdentalConstants.VOXEL_PLANE_NAME,voxel_idx,i)
            z_location = i * z_spacing

            voxel_plane_obj = AddPlaneObject(voxel_plane_name, voxel_plane_mesh.copy(), BdentalConstants.DICOM_VIZ_COLLECTION_NAME)
            voxel_plane_obj.location = [dim_x/2, dim_y/2, z_location]
            voxel_plane_obj[BdentalConstants.BDENTAL_TYPE_TAG] = BdentalConstants.VOXEL_PLANE_TYPE
            voxel_plane_obj["uid"] = self.uid
            
            ##########################################
            # Add Material :
            plane_mat_name = get_incremental_name(BdentalConstants.VOXEL_PLANE_MAT_NAME, voxel_idx,i)
        ...
        voxel_planes = [o for o in bpy.context.scene.objects if \
                        o.get(BdentalConstants.BDENTAL_TYPE_TAG)==BdentalConstants.VOXEL_PLANE_TYPE and \
                            o.get("uid") == self.uid]
        ...
        MoveToCollection(voxel_vizualization_object, BdentalConstants.DICOM_VIZ_COLLECTION_NAME)
        voxel_vizualization_object[BdentalConstants.BDENTAL_TYPE_TAG] = BdentalConstants.VOXEL_OBJECT_TYPE
        voxel_vizualization_object[BdentalConstants.MHA_PATH_TAG] = self.mha_path
        ...
        s3d.shading.xray_alpha = BdentalConstants.XRAY_ALPHA
        for scr in bpy.data.screens:
            # if scr.name in ["Layout", "Scripting", "Shading"]:
        ...
        self._props.GroupNodeName = BdentalConstants.BDENTAL_VOXEL_SHADER
        ...
        WminNode.default_value = BdentalConstants.WMIN
        WmaxNode = voxel_node.nodes["WmaxNode"].outputs[0]
        WmaxNode.default_value = BdentalConstants.WMAX
    ...
    def execute(self, context):
        ...
        bpy.ops.wm.bdental_volume_slicer()

        message = ["finished."]
        BDENTAL_GpuDrawText(message_list=message, sleep_time=2)
...
def update_slices(scene):
    ...
    if not slices_pointer or not slices_pointer.get(BdentalConstants.BDENTAL_TYPE_TAG) == BdentalConstants.SLICES_POINTER_TYPE or not slices_pointer.select_get():
        return
    if not IMAGE3D :
        slices_pointer_mha_path = slices_pointer.get(BdentalConstants.MHA_PATH_TAG)
    ...
    props = scene.BDENTAL_Props 
    slices_dir = props.SlicesDir
    ...
    slice_planes = [obj for obj in scene.objects if obj.get(BdentalConstants.BDENTAL_TYPE_TAG) == BdentalConstants.SLICE_PLANE_TYPE]
...
class BDENTAL_OT_VolumeSlicer(bpy.types.Operator):
    """Add Volume Slices"""

    bl_idname = "wm.bdental_volume_slicer"
    ...
    def poll(cls, context):
        if not context.object or not context.object.select_get():
            return False
        return context.object.get(BdentalConstants.BDENTAL_TYPE_TAG) in [BdentalConstants.VOXEL_OBJECT_TYPE, BdentalConstants.PCD_OBJECT_TYPE]
    ...
    def add_cam_to_plane(self, context,p):
        ...
        transform.translation = Vector((0, 0, BdentalConstants.CAM_DISTANCE))

        Cam.matrix_world = transform

        Cam.data.clip_start = BdentalConstants.CAM_DISTANCE - BdentalConstants.CAM_CLIP_OFFSET
        Cam.data.clip_end = BdentalConstants.CAM_DISTANCE + BdentalConstants.CAM_CLIP_OFFSET

        Cam.hide_set(True)
        MoveToCollection(obj=Cam, CollName=BdentalConstants.SLICES_COLLECTION_NAME)
    ...
    def set_image3d(self, context) :
        global IMAGE3D
        message = []
        scn_mha_path = context.scene.get(BdentalConstants.MHA_PATH_TAG)
        if not scn_mha_path or not exists(AbsPath(scn_mha_path)) :
            message = ["bdental dicom data file not found, please read dicom and retry !"]
        obj = context.object
        if obj and obj.select_get() and obj.get(BdentalConstants.MHA_PATH_TAG):  
            mha_path_rel = obj.get(BdentalConstants.MHA_PATH_TAG)
            if not mha_path_rel == scn_mha_path :
                context.scene[BdentalConstants.MHA_PATH_TAG] = mha_path_rel
            ...
            if not IMAGE3D or self.source_mha != scn_mha_path :
                context.scene[BdentalConstants.MHA_PATH_TAG] = self.source_mha
    ...
    def add_slices_pointer(self, context):
        ...
        slices_pointer.name = BdentalConstants.SLICES_POINTER_NAME
        slices_pointer[BdentalConstants.BDENTAL_TYPE_TAG] = BdentalConstants.SLICES_POINTER_TYPE
        slices_pointer[BdentalConstants.MHA_PATH_TAG] = self.source_mha
        ...
        MoveToCollection(slices_pointer, BdentalConstants.SLICES_POINTER_COLLECTION_NAME)
        return slices_pointer
    
    def add_slices(self,context):
        
        scn = context.scene
        slices_dir = self.props.SlicesDir
        ...
        slice_planes_names = [  BdentalConstants.AXIAL_SLICE_NAME,
                                BdentalConstants.CORONAL_SLICE_NAME,
                                BdentalConstants.SAGITTAL_SLICE_NAME]
        slice_planes_obj = []
        cams = []
        ...
        for s_name in slice_planes_names:
            ...
            p[BdentalConstants.BDENTAL_TYPE_TAG] = BdentalConstants.SLICE_PLANE_TYPE
            p.rotation_mode = "XYZ"
            MoveToCollection(obj=p, CollName=BdentalConstants.SLICES_COLLECTION_NAME)
        ...
        update_slices(scn)
        ######################################################
        slices_node_group = bpy.data.node_groups.get(BdentalConstants.SLICES_SHADER_NAME)
        if not slices_node_group:
            filepath = join(BdentalConstants.DATA_BLEND_FILE, "NodeTree", BdentalConstants.SLICES_SHADER_NAME)
            directory = join(BdentalConstants.DATA_BLEND_FILE, "NodeTree")
            filename = BdentalConstants.SLICES_SHADER_NAME
            bpy.ops.wm.append(filepath=filepath,
                            filename=filename, directory=directory)
            slices_node_group = bpy.data.node_groups.get(BdentalConstants.SLICES_SHADER_NAME)
        ...
        for p in slice_planes_obj :
            ...
            if not BlenderImage:
                ...
                BlenderImage = bpy.data.images.get(ImageName)
                BlenderImage[BdentalConstants.BDENTAL_TYPE_TAG] = BdentalConstants.SLICE_IMAGE_TYPE
    ...
    def remove_old_stuff(self, context):
        for obj in context.scene.objects :
            if obj.get(BdentalConstants.BDENTAL_TYPE_TAG) == BdentalConstants.SLICES_POINTER_TYPE:
                remove_all_pointer_child_of(obj)
                bpy.data.objects.remove(obj)
        coll = bpy.data.collections.get(BdentalConstants.SLICES_COLLECTION_NAME)
        if coll:
            for obj in coll.objects:
                bpy.data.objects.remove(obj)
            bpy.data.collections.remove(coll)

        images = [img for img in bpy.data.images if img.get(BdentalConstants.BDENTAL_TYPE_TAG) == BdentalConstants.SLICE_IMAGE_TYPE]
        for img in images:
    ...
    def execute(self, context):
        ...
        self.main_ws = bpy.data.workspaces.get(BdentalConstants.MAIN_WORKSPACE_NAME)
        self.ws_slicer = bpy.data.workspaces.get(BdentalConstants.SLICER_WORKSPACE_NAME)

        if not (self.main_ws and self.ws_slicer):
            BDENTAL_GpuDrawText(
                message_list=["Bdental Workspaces not found"], rect_color=BdentalColors.red, sleep_time=2
            )
            return {"CANCELLED"}
        self.source_mha = self.vis_object.get(BdentalConstants.MHA_PATH_TAG)
        if not self.source_mha or not exists(AbsPath(self.source_mha)):
            BDENTAL_GpuDrawText(
                message_list=["Bdental MHA file not found"], rect_color=BdentalColors.red, sleep_time=2
            )
            return {"CANCELLED"}
        context.scene[BdentalConstants.MHA_PATH_TAG] = self.source_mha
        IMAGE3D = sitk.ReadImage(AbsPath(self.source_mha))

        self.props = context.scene.BDENTAL_Props
    ...
    def set_mpr(self,context):
        ...
        if not slicer_ws_success:
            BDENTAL_GpuDrawText(
                message_list=["Bdental Slicer areas not found"], rect_color=BdentalColors.red, sleep_time=2
            )
            return {"CANCELLED"}
        ...
        try :
            ...
            print(er)
            BDENTAL_GpuDrawText(
                message_list=["Bdental Main area 3D not found"], rect_color=BdentalColors.red, sleep_time=2
            )
            return {"CANCELLED"}
        ...
        ws_slicer_mpr_to_keep_sentences = [BdentalConstants.SLICES_COLLECTION_NAME, BdentalConstants.SLICES_POINTER_COLLECTION_NAME, BdentalConstants.GUIDE_COMPONENTS_COLLECTION_NAME]
        ...
        ws_slicer_area_3d_to_hide_coll_names = [BdentalConstants.SLICES_COLLECTION_NAME]
        ws_main_area_3d_to_hide_coll_names = [BdentalConstants.SLICES_COLLECTION_NAME]
        ...
        #####################################################
        # hide collections from Bdental Main Workspace/area 3d :
        ...
        if self.open_slices_view:
            try :
                ...
                bdental_log([f"open bdental slicer workspace error : {e}"])
                pass
        bpy.ops.object.select_all(action="DESELECT")
...
class BDENTAL_OT_DicomToMesh(bpy.types.Operator):
    """Dicom to Mesh segmentation"""

    bl_idname = "wm.bdental_dicom_to_mesh"
    ...
    def poll(cls, context):
        ...
        return context.object.get(BdentalConstants.BDENTAL_TYPE_TAG) in[
    ...
    def set_image3d(self, context) :
        global IMAGE3D
        if not IMAGE3D or self.source_mha != context.scene.get(BdentalConstants.MHA_PATH_TAG) :
            context.scene[BdentalConstants.MHA_PATH_TAG] = self.source_mha
            IMAGE3D = sitk.ReadImage(AbsPath(self.source_mha))
    ...
    def dicom_to_mesh(self):
        ...
        threshold255 = HuTo255(self.props.ThresholdMin, BdentalConstants.WMIN, BdentalConstants.WMAX)
        if threshold255 == 0: threshold255+=1
        elif threshold255 == 255: threshold255-=1
        ...
        error_message, dicom_mesh = create_blender_mesh_from_marching_cubes_fast(name=f"{BdentalConstants.DICOM_MESH_NAME}#{self.idx}({self.props.ThresholdMin})",
                                       marching_cubes = _marching_cubes,
                                        transform = None)
        if error_message :
            return error_message, dicom_mesh

        dicom_mesh[BdentalConstants.BDENTAL_TYPE_TAG] = BdentalConstants.DICOM_MESH_TYPE
        dicom_mesh["idx"] = self.idx
        dicom_mesh["uid"] = self.uid
        dicom_mesh[BdentalConstants.MHA_PATH_TAG] = self.source_mha
    ...
    def execute(self, context):
        ...
        self.props = context.scene.BDENTAL_Props
        self.source_mha = context.object[BdentalConstants.MHA_PATH_TAG]
        ...
        BDENTAL_GpuDrawText(message_list=["Finished"], rect_color=BdentalColors.green, sleep_time=2)
        return {"FINISHED"}
...
class BDENTAL_OT_MultiTreshSegment(bpy.types.Operator):
    ...
    def DicomToStl(self, Segment, Image3D):
        ...
        Treshold255 = HuTo255(Hu=SegmentTreshold, Wmin=BdentalConstants.WMIN, Wmax=BdentalConstants.WMAX)
        if Treshold255 == 0:
            Treshold255 = 1
    ...
    def poll(cls, context):
        if not context.object or not context.object.select_get():
            return False
        return context.object.get(BdentalConstants.BDENTAL_TYPE_TAG) == BdentalConstants.VOXEL_OBJECT_TYPE
...
class BDENTAL_OT_AddImplant(bpy.types.Operator):
    ...
    item_choice : EnumProperty(items=set_enum_items(["Implant","Fixing Pin"]), description="Implant or Fixing Pin enum", default="Implant") # type: ignore
    ...
    def poll(cls, context):
        ...
        return context.object.get(BdentalConstants.BDENTAL_TYPE_TAG) == BdentalConstants.SLICES_POINTER_TYPE
    ...
    def execute(self, context):
        ...
        if self.item_choice == "Implant" :
            ...
            name = f"{BdentalConstants.BDENTAL_IMPLANT_NAME_PREFFIX}_{self.tooth_number}_{self.implant_diameter}x{self.implant_lenght}mm"
            implant_exists_for_same_tooth = [
                obj for obj in context.scene.objects if obj.get(BdentalConstants.BDENTAL_TYPE_TAG)==BdentalConstants.BDENTAL_IMPLANT_TYPE and obj.get(BdentalConstants.BDENTAL_IMPLANT_REMOVE_CODE_TAG) == self.tooth_number]
            ...
            implant[BdentalConstants.BDENTAL_TYPE_TAG] = BdentalConstants.BDENTAL_IMPLANT_TYPE
            implant[BdentalConstants.BDENTAL_IMPLANT_REMOVE_CODE_TAG] = self.tooth_number
            ...
            if self.safe_zone :
                ...
                safe_zone[BdentalConstants.BDENTAL_TYPE_TAG] = BdentalConstants.IMPLANT_SAFE_ZONE_TYPE
                safe_zone[BdentalConstants.BDENTAL_IMPLANT_REMOVE_CODE_TAG] = self.tooth_number
            ...
            if self.sleeve :
                ...
                sleeve[BdentalConstants.BDENTAL_TYPE_TAG] = BdentalConstants.IMPLANT_SLEEVE_TYPE
                sleeve[BdentalConstants.BDENTAL_IMPLANT_REMOVE_CODE_TAG] = self.tooth_number
            ...
            if self.pin :
                ...
                pin[BdentalConstants.BDENTAL_TYPE_TAG] = BdentalConstants.IMPLANT_PIN_TYPE
                pin[BdentalConstants.BDENTAL_IMPLANT_REMOVE_CODE_TAG] = self.tooth_number
...
class BDENTAL_OT_AddImplantSleeve(bpy.types.Operator):
    ...
    def poll(cls, context):
        if not context.object or not context.object.select_get():
            return False
        return context.object.get(BdentalConstants.BDENTAL_TYPE_TAG) == BdentalConstants.BDENTAL_IMPLANT_TYPE
...
class BDENTAL_OT_AddSleeve(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for obj in [Pin, Sleeve]:
            MoveToCollection(obj, BdentalConstants.GUIDE_COMPONENTS_COLLECTION_NAME)
...
class BDENTAL_OT_RemoveImplant(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        remove_code = implant.get(BdentalConstants.BDENTAL_IMPLANT_REMOVE_CODE_TAG)
        for o in context.scene.objects :
            if o.get(BdentalConstants.BDENTAL_IMPLANT_REMOVE_CODE_TAG) == remove_code :
                bpy.data.objects.remove(o)
        
        txt = [f"Bdental implant {remove_code} removed."]
        BDENTAL_GpuDrawText(message_list=txt, rect_color=BdentalColors.green, sleep_time=1)
        return {"FINISHED"}
...
#         context.object.get(BdentalConstants.BDENTAL_TYPE_TAG) == BdentalConstants.BDENTAL_IMPLANT_TYPE
...
#             obj[BdentalConstants.IMPLANT_MAT_TAG] = mat.name
#         mat_bdental_locked = bpy.data.materials.get(
#             BdentalConstants.IMPLANT_LOCKED_MAT_NAME) or bpy.data.materials.new(BdentalConstants.IMPLANT_LOCKED_MAT_NAME)
...
class BDENTAL_OT_LockObjectToPointer(bpy.types.Operator):
    """add child constraint to slices pointer"""

    bl_idname = "wm.bdental_lock_object_to_pointer"
    bl_label = "Lock to Pointer"
    ...
    def poll(cls, context):
        ...
        if not context.object or not context.object.select_get() or \
            context.object.get(BdentalConstants.BDENTAL_TYPE_TAG)\
             in [BdentalConstants.SLICE_PLANE_TYPE, BdentalConstants.SLICES_POINTER_TYPE] : return False
    ...
    def execute(self, context):
        ...
        if mat :
            mat.use_fake_user = True
            obj[BdentalConstants.PREVIOUS_ACTIVE_MAT_NAME_TAG] = mat.name
        bdental_locked_to_pointer_mat = bpy.data.materials.get(
            BdentalConstants.LOCKED_TO_POINTER_MAT_NAME) or bpy.data.materials.new(BdentalConstants.LOCKED_TO_POINTER_MAT_NAME)
        bdental_locked_to_pointer_mat.use_fake_user = True
        bdental_locked_to_pointer_mat.diffuse_color = (1, 0, 0, 1)  # red color
        bdental_locked_to_pointer_mat.use_nodes = True
        # mat_bdental_locked.node_tree.nodes["Principled BSDF"].inputs[19].default_value = (
        #     1, 0, 0, 1)  # red color
        nodes = bdental_locked_to_pointer_mat.node_tree.nodes
        pbsdf_node = [n for n in nodes if n.type =='BSDF_PRINCIPLED'][0]
        pbsdf_node.inputs[0].default_value = (
            1, 0, 0, 1)  # red color
        bpy.ops.object.material_slot_remove_all()
        obj.active_material = bdental_locked_to_pointer_mat
...
class BDENTAL_OT_UnlockObjectFromPointer(bpy.types.Operator):
    """remove child constraint to slices pointer"""

    bl_idname = "wm.bdental_unlock_object_from_pointer"
    ...
    def poll(cls, context):
        ...
        if not context.object or not context.object.select_get() or context.object.get(BdentalConstants.BDENTAL_TYPE_TAG)\
             in [BdentalConstants.SLICE_PLANE_TYPE, BdentalConstants.SLICES_POINTER_TYPE] : return False
...
class BDENTAL_OT_ObjectToPointer(bpy.types.Operator):
    """move and align object to pointer """

    bl_idname = "wm.bdental_object_to_pointer"
    bl_label = "To Pointer"
    ...
    def poll(cls, context):
        ...
        if context.object.get(BdentalConstants.BDENTAL_TYPE_TAG) in (
            BdentalConstants.SLICES_POINTER_TYPE,
            BdentalConstants.SLICE_PLANE_TYPE,
            ) : return False
        return True
    ...
    def execute(self, context):
        slices_pointer_check_list = [
            obj for obj in context.scene.objects if  obj.get(BdentalConstants.BDENTAL_TYPE_TAG) == BdentalConstants.SLICES_POINTER_TYPE]
        if not slices_pointer_check_list:
            BDENTAL_GpuDrawText(message_list=["No Slices Pointer Found!"], rect_color=BdentalColors.red, sleep_time=2)
            return {"CANCELLED"}
        slices_pointer = slices_pointer_check_list[0]
...
class BDENTAL_OT_PointerToImplant(bpy.types.Operator):
    ...
    def poll(cls, context):
        ...
        if not context.object.get(BdentalConstants.BDENTAL_TYPE_TAG) in [
            BdentalConstants.BDENTAL_IMPLANT_TYPE,
            BdentalConstants.FIXING_SLEEVE_TYPE,
...
class BDENTAL_OT_PointerToActive(bpy.types.Operator):
    """move pointer to active object"""

    bl_idname = "wm.bdental_pointer_to_active"
    bl_label = "Pointer to Active"
    @classmethod
    def poll(cls, context):
        return context.object and context.object.select_get() and \
        context.object.get(BdentalConstants.BDENTAL_TYPE_TAG) != BdentalConstants.SLICES_POINTER_TYPE
...
#         for obj in context.scene.objects :
#             if obj.get(BdentalConstants.BDENTAL_TYPE_TAG) in (BdentalConstants.SLICE_PLANE_TYPE, BdentalConstants.SLICES_POINTER_TYPE) :
#                 continue
...
#                     context.view_layer.objects.active = obj
#                     bpy.ops.wm.bdental_unlock_object_from_pointer()
#                     if obj.get(BdentalConstants.BDENTAL_TYPE_TAG) == target_object_type :
...
class BDENTAL_OT_FlyToImplantOrFixingSleeve(bpy.types.Operator):
    """move pointer to previous or next implant or fixing sleeve/pin iteratively"""

    bl_idname = "wm.bdental_fly_to_implant_or_fixing_sleeve"
    ...
    def execute(self, context):
        ...
        remove_all_pointer_child_of(slices_pointer)
...
class BDENTAL_OT_AlignObjectsAxes(bpy.types.Operator):
    ...
    bl_idname = "wm.bdental_align_objects_axes"
    bl_label = "Align Axes"
    bl_options = {'REGISTER', 'UNDO'}
...
class BDENTAL_OT_SplintGuide(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        self.base_mesh_duplicate[BdentalConstants.BDENTAL_TYPE_TAG] = "bdental_splint"
...
class BDENTAL_OT_SplintGuideGeom(bpy.types.Operator):
    ...
    def splint(self, context):
        ...
        gn = append_group_nodes(BdentalConstants.BDENTAL_VOLUME_NODE_NAME)
    ...
    def modal(self, context, event):
        if not event.type in ["RET", "ESC", "LEFTMOUSE", "DEL"]:
            ...
            if event.value == ("PRESS"):
                ...
                message = ["CANCELLED"]
                BDENTAL_GpuDrawText(message_list=message,rect_color=BdentalColors.red,sleep_time=1)
                return {"CANCELLED"}
            ...
            if event.value == ("PRESS"):
                ...
                message = ["FINISHED."]
                BDENTAL_GpuDrawText(message_list=message,rect_color=BdentalColors.green,sleep_time=1)
                return {"FINISHED"}
...
class BDENTAL_OT_GuideFinalise(bpy.types.Operator):
    ...
    @classmethod
    def poll(cls, context):
        coll = bpy.data.collections.get(BdentalConstants.GUIDE_COMPONENTS_COLLECTION_NAME)
    ...
    def execute(self, context):
        ...
        coll = bpy.data.collections[BdentalConstants.GUIDE_COMPONENTS_COLLECTION_NAME]
        ...
        if not cut_components:
            ...
            guide[BdentalConstants.BDENTAL_TYPE_TAG] = "bdental_guide"
        ...
        BDENTAL_GpuDrawText(message_list=["Guide Finalised !"],rect_color=BdentalColors.green,sleep_time=2)
        bdental_log([f"Guide Finalised in {round(end-start)} seconds."])
        
        return {"FINISHED"}

class BDENTAL_OT_GuideFinaliseGeonodes(bpy.types.Operator):
    """Guide finalise using geonodes boolean"""

    bl_idname = "wm.bdental_guide_finalise_geonodes"
    ...
    def execute(self, context):
        ...
        for obj in bpy.context.scene.objects :
            if BdentalConstants.GUIDE_NAME in obj.name :
                suffix+=1
        guide = AppendObject(BdentalConstants.GUIDE_NAME,"Guide Collection")
        bpy.ops.object.select_all(action='DESELECT')
        context.view_layer.objects.active = guide
        guide.select_set(True)
        guide.name += f"_{suffix}"
        gn = bpy.data.node_groups.get(BdentalConstants.BOOL_NODE)
...
class BDENTAL_OT_AddGuideCuttersFromSleeves(bpy.types.Operator):
    ...
    def poll(cls, context):
        ...
        sleeves_checklist = [obj for obj in context.scene.objects if obj.get(
            BdentalConstants.BDENTAL_TYPE_TAG) and "bdental_sleeve" in obj.get(BdentalConstants.BDENTAL_TYPE_TAG)]
    ...
    def invoke(self, context, event):
        if context.space_data.type == "VIEW_3D":
            ...
            for sleeve in self.sleeves_checklist:
                ...
                bpy.ops.object.duplicate_move()
                cutter = context.object
                cutter[BdentalConstants.BDENTAL_TYPE_TAG] = "bdental_guide_cutter"
        ...
        if not event.type in {'ESC', 'RET'}:
            ...
            if event.value == 'RELEASE':
                ...
                if self.guide_component == "Cube":
                    ...
                    MoveToCollection(cube, BdentalConstants.GUIDE_COMPONENTS_COLLECTION_NAME)
                    ...
                    MoveToCollection(sphere, BdentalConstants.GUIDE_COMPONENTS_COLLECTION_NAME)
                    ...
                    MoveToCollection(cylinder, BdentalConstants.GUIDE_COMPONENTS_COLLECTION_NAME)
...
class BDENTAL_OT_AddCustomSleeveCutter(bpy.types.Operator):
                ...
                if not success :
                    ...
                    for i,obj in enumerate(objs) :
                        ...
                        if obj in self.implts :
                            tooth_number = obj.get(BdentalConstants.BDENTAL_IMPLANT_REMOVE_CODE_TAG)
                            if tooth_number < 31 :
                                flip = True
                        bpy.ops.object.select_all(action='DESELECT')
                        context.view_layer.objects.active = obj
                        ...
                        asset_dup[BdentalConstants.BDENTAL_TYPE_TAG]="bdental_custom_sleeve cutter"
                        MoveToCollection(asset_dup, BdentalConstants.GUIDE_COMPONENTS_COLLECTION_NAME)
    ...
    def defer(self):
        ...
        try:
            params.asset_library_ref = BdentalConstants.BDENTAL_LIB_NAME
    ...
    def invoke(self, context, event):
        ...
        if context.space_data.type == "VIEW_3D":
            self.implts = [o for o in context.scene.objects[:] if o.get(BdentalConstants.BDENTAL_TYPE_TAG) == BdentalConstants.BDENTAL_IMPLANT_TYPE ]
            self.fixing_sleeves = [o for o in context.scene.objects[:] if o.get(BdentalConstants.BDENTAL_TYPE_TAG) == BdentalConstants.FIXING_SLEEVE_TYPE ]
    ...
    def modal(self, context, event):
        if not event.type in {'ESC', 'RET'}:
            ...
            if event.value == 'RELEASE':
                ...
                #Add pin
                bpy.ops.object.select_all(action="DESELECT")
                pin = AppendObject("fixing_pin", coll_name=BdentalConstants.GUIDE_COMPONENTS_COLLECTION_NAME)
                pin.name = f"Fixing_Pin({n+1})"
                pin[BdentalConstants.BDENTAL_TYPE_TAG] =BdentalConstants.FIXING_PIN_TYPE
                ...
                bpy.ops.object.select_all(action="DESELECT")
                sleeve = AppendObject(
                    "sleeve", coll_name=BdentalConstants.GUIDE_COMPONENTS_COLLECTION_NAME)
                sleeve.name = f"_ADD_Fixing_Sleeve({n+1})"
                sleeve[BdentalConstants.BDENTAL_TYPE_TAG] = BdentalConstants.FIXING_SLEEVE_TYPE
                ...
                bpy.ops.wm.bdental_pointer_to_active()
                bpy.ops.wm.bdental_lock_object_to_pointer()
...
class BDENTAL_OT_BlockoutNew(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        message = ["Finished."]
        BDENTAL_GpuDrawText(message_list=message, rect_color=BdentalColors.green, sleep_time=2)
        return {"FINISHED"}
...
class BDENTAL_OT_AutoAlign(bpy.types.Operator):
    ...
    def execute(self, context):
        # binary = shlex.quote(BdentalConstants.MESH_REG_AUTO_PATH)
        # print("binary : ", binary)
        ...
        # cmd = f'"{BdentalConstants.MESH_REG_AUTO_PATH}" "{self.src_path}" "{self.tgt_path}" --transform_output "{self.transform_path}"'
        ...
        cmd = [BdentalConstants.MESH_REG_AUTO_PATH, self.src_path, self.tgt_path, "--transform_output", self.transform_path]
        if self.mode == "global" :
...
class BDENTAL_OT_RibbonCutterAdd(bpy.types.Operator):
    ...
    def add_curve_cutter(self, context):
        ...
        self.cutter[BdentalConstants.BDENTAL_TYPE_TAG] = "curvecutter3"
...
class BDENTAL_OT_RibbonCutter_Perform_Cut(bpy.types.Operator):
    ...
    @classmethod
    def poll(cls, context):
        cutters = [obj for obj in context.scene.objects if obj.get(BdentalConstants.BDENTAL_TYPE_TAG) == "curvecutter3"]
        return cutters
    ...
    def execute(self, context):
        ...
        allcutters = [obj for obj in context.scene.objects if obj.get(BdentalConstants.BDENTAL_TYPE_TAG) == "curvecutter3"]
        for c in allcutters :
...
class BDENTAL_OT_CurveCutter1_New_Perform_Cut(bpy.types.Operator):
    ...
    @classmethod
    def poll(cls, context):
        cutters = [obj for obj in context.scene.objects if obj.get(BdentalConstants.BDENTAL_TYPE_TAG) == "curvecutter1"]
        return cutters
    ...
    def execute(self, context):
        ...
        allcurvecutters1 = [obj for obj in context.scene.objects if obj.get(BdentalConstants.BDENTAL_TYPE_TAG) == "curvecutter1"]
...
class BDENTAL_OT_CurveCutterAdd2(bpy.types.Operator):
    ...
    def add_curve_cutter(self, context):
        ...
        self.cutter = bpy.context.view_layer.objects.active
        self.cutter.name = "BDENTAL_Cutter"
        self.cutter[BdentalConstants.BDENTAL_TYPE_TAG] = "curvecutter2"
        self.cutter["bdental_target"] = self.base_mesh.name
...
class BDENTAL_OT_CurveCutter2_Cut_New(bpy.types.Operator):
    ...
    @classmethod
    def poll(cls, context):
        cutters = [obj for obj in context.scene.objects if obj.get(BdentalConstants.BDENTAL_TYPE_TAG) == "curvecutter2"]
        return cutters
    ...
    def execute(self, context):
        ...
        allcurvecutters2 = [obj for obj in context.scene.objects if obj.get(BdentalConstants.BDENTAL_TYPE_TAG) == "curvecutter2"]
...
class BDENTAL_OT_SlicesPointerSelect(bpy.types.Operator):
    ...
    @classmethod
    def poll(cls, context):
        return [obj for obj in bpy.data.objects if obj.get(BdentalConstants.BDENTAL_TYPE_TAG)==BdentalConstants.SLICES_POINTER_TYPE]
        
    def execute(self, context):
        
        slices_pointer = [
            obj for obj in bpy.data.objects if obj.get(BdentalConstants.BDENTAL_TYPE_TAG)==BdentalConstants.SLICES_POINTER_TYPE][0]
        
        hide_collection(_hide=False, colname=BdentalConstants.SLICES_POINTER_COLLECTION_NAME)
...
class BDENTAL_OT_AddReferencePlanes(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        if context.space_data.type == "VIEW_3D":
            ...
            # self.MarkupVoxelMode = (self.TargetObject.get(BdentalConstants.BDENTAL_TYPE_TAG)=="CT_Voxel")
...
classes = [
    ...
    BDENTAL_OT_AlignObjectsAxes,
    BDENTAL_OT_AddImplantSleeve,
    BDENTAL_OT_AddFixingPin,
    BDENTAL_OT_LockObjectToPointer,
    BDENTAL_OT_UnlockObjectFromPointer,
    # BDENTAL_OT_ImplantToPointer,
    BDENTAL_OT_ObjectToPointer,
    # BDENTAL_OT_PointerToImplant,
    BDENTAL_OT_PointerToActive,
    BDENTAL_OT_FlyToImplantOrFixingSleeve,
    # BDENTAL_OT_FlyNext,
